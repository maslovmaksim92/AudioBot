...
class BitrixService:
    ...
    async def get_deals(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if all_deals:
                ...
                # ОТЛАДКА: проверяем первую сделку
                if len(all_deals) > 0:
                    first_deal = all_deals[0]
                    logger.info(f"🔍 FIRST DEAL DEBUG: ID={first_deal.get('ID')}, COMPANY_ID={first_deal.get('COMPANY_ID')}")
                ...
                logger.info(f"📊 Processed {len(processed_deals)} deals, starting companies enrichment...")
                
                # Обогащаем управляющими компаниями
                enriched_deals = await self._enrich_with_management_companies(processed_deals)
                
                logger.info(f"✅ Final processed and enriched {len(enriched_deals)} houses")
    ...
    def _get_cleaning_type_name(self, type_id: str) -> str:
        ...
        # РЕАЛЬНЫЙ маппинг типов уборки из Bitrix24 CRM API (через crm.deal.fields.json)
        # UF_CRM_1741592855565 (Тип уборки 1) и UF_CRM_1741592945060 (Тип уборки 2)
        cleaning_types = {
            # Тип уборки 1
            ...
            # Тип уборки 2
            '2472': 'Подметание лестничных площадок и маршей всех этажей, влажная уборка 1 этажа и лифта (при наличии); Профилактическая дезинфекция МОП',
            '2474': 'Влажная уборка лестничных площадок всех этажей и лифта (при наличии); Профилактическая дезинфекция МОП',
            '2476': 'Влажная уборка 1 этажа и лифта (при наличии); Профилактическая дезинфекция МОП'
        }
