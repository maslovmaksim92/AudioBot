...
CORS_ORIGINS_RAW = os.environ.get('CORS_ORIGINS', 'https://crmunified.preview.emergentagent.com,https://audiobot-qci2.onrender.com,http://localhost:3000')
CORS_ORIGINS = [origin.strip() for origin in CORS_ORIGINS_RAW.split(',') if origin.strip()]

# Безопасные дефолтные origins если переменная пустая
if not CORS_ORIGINS:
    CORS_ORIGINS = [
        "https://crmunified.preview.emergentagent.com",
        "https://audiobot-qci2.onrender.com",
        "http://localhost:3000"  # Добавляем localhost для разработки
    ]
