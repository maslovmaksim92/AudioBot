...
async def get_cleaning_houses(
    ...
    try:
        ...
        for deal in deals:
            ...
            # ПРАВИЛЬНЫЙ ГРАФИК УБОРКИ СЕНТЯБРЯ из Bitrix24
            september_schedule = _parse_september_schedule(deal)
            
            # Упрощенные графики для фильтрации
            cleaning_weeks = [1, 2, 3] if apartments_count and apartments_count > 50 else [1, 2]
            cleaning_days = ['Понедельник', 'Среда'] if apartments_count and apartments_count > 100 else ['Вторник']
...
def _get_cleaning_type_name(type_id) -> str:
    ...
    # Расширенный маппинг ID типов уборки на их названия
    type_mapping = {
        ...
        '4': 'Уборка подъездов',
        '5': 'Уборка территории',
        '2468': 'Влажная уборка лестничных площадок',
        '2476': 'Подметание маршей и дезинфекция',
        '2477': 'Генеральная уборка МОП',
        '2478': 'Профилактическая дезинфекция',
        '2479': 'Уборка лифтов',
        '2480': 'Мытье стекол в подъездах',
        '2481': 'Вывоз мусора',
        '2482': 'Уборка подвалов и чердаков'
    }
    
    return type_mapping.get(str(type_id), f"Плановая уборка (код {type_id})")
