    ...
    async def _load_deals_from_category(self, category_id: str) -> List[Dict[str, Any]]:
        ...
        while True:
            params = {
                ...
                # Сентябрь 2025
                'select[12]': 'UF_CRM_1741592774017', # Дата уборки 1 Сентябрь
                'select[13]': 'UF_CRM_1741592855565', # Тип уборки 1 Сентябрь
                'select[14]': 'UF_CRM_1741592892232', # Дата уборки 2 Сентябрь
                'select[15]': 'UF_CRM_1741592945060', # Тип уборки 2 Сентябрь
                # Октябрь 2025
                'select[16]': 'UF_CRM_1741593004888', # Дата уборки 1 Октябрь
                'select[17]': 'UF_CRM_1741593047994', # Тип уборки 1 Октябрь
                'select[18]': 'UF_CRM_1741593067418', # Дата уборки 2 Октябрь
                'select[19]': 'UF_CRM_1741593115407', # Тип уборки 2 Октябрь
                # Ноябрь 2025
                'select[20]': 'UF_CRM_1741593156926', # Дата уборки 1 Ноябрь
                'select[21]': 'UF_CRM_1741593210242', # Тип уборки 1 Ноябрь
                'select[22]': 'UF_CRM_1741593231558', # Дата уборки 2 Ноябрь
                'select[23]': 'UF_CRM_1741593285121', # Тип уборки 2 Ноябрь
                # Декабрь 2025
                'select[24]': 'UF_CRM_1741593340713', # Дата уборки 1 Декабрь
                'select[25]': 'UF_CRM_1741593387667', # Тип уборки 1 Декабрь
                'select[26]': 'UF_CRM_1741593408621', # Дата уборки 2 Декабрь
                'select[27]': 'UF_CRM_1741593452062', # Тип уборки 2 Декабрь
