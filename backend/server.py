...
# DB / Vector (optional for AI features)
from sqlalchemy.ext.asyncio import create_async_engine, AsyncSession, async_sessionmaker
from sqlalchemy.orm import declarative_base
from sqlalchemy import Column, Integer, String, Text, DateTime, ForeignKey
from sqlalchemy import text as sa_text

# LLM / Files (optional for AI Training)
...
# Build cleaning_dates map (months for 2025) from UF fields
def _build_cleaning_dates(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if d.get("UF_CRM_1741592774017") or d.get("UF_CRM_1741592855565"):
        ...
        out["september_1"] = {"dates": dates if isinstance(dates, list) else ([dates] if dates else []), "type": str(d.get("UF_CRM_1741592855565") or "")}
    if d.get("UF_CRM_1741592892232") or d.get("UF_CRM_1741592945060"):
        dates = d.get("UF_CRM_1741592892232");
        out["september_2"] = {"dates": dates if isinstance(dates, list) else ([dates] if dates else []), "type": str(d.get("UF_CRM_1741592945060") or "")}
    # October 2025
    if d.get("UF_CRM_1741593004888") or d.get("UF_CRM_1741593047994"):
        dates = d.get("UF_CRM_1741593004888");
        out["october_1"] = {"dates": dates if isinstance(dates, list) else ([dates] if dates else []), "type": str(d.get("UF_CRM_1741593047994") or "")}
    if d.get("UF_CRM_1741593067418") or d.get("UF_CRM_1741593115407"):
        dates = d.get("UF_CRM_1741593067418");
        out["october_2"] = {"dates": dates if isinstance(dates, list) else ([dates] if dates else []), "type": str(d.get("UF_CRM_1741593115407") or "")}
    # November 2025
    if d.get("UF_CRM_1741593156926") or d.get("UF_CRM_1741593210242"):
        dates = d.get("UF_CRM_1741593156926");
        out["november_1"] = {"dates": dates if isinstance(dates, list) else ([dates] if dates else []), "type": str(d.get("UF_CRM_1741593210242") or "")}
    if d.get("UF_CRM_1741593231558") or d.get("UF_CRM_1741593285121"):
        dates = d.get("UF_CRM_1741593231558");
        out["november_2"] = {"dates": dates if isinstance(dates, list) else ([dates] if dates else []), "type": str(d.get("UF_CRM_1741593285121") or "")}
    # December 2025
    if d.get("UF_CRM_1741593340713") or d.get("UF_CRM_1741593387667"):
        dates = d.get("UF_CRM_1741593340713");
        out["december_1"] = {"dates": dates if isinstance(dates, list) else ([dates] if dates else []), "type": str(d.get("UF_CRM_1741593387667") or "")}
    if d.get("UF_CRM_1741593408621") or d.get("UF_CRM_1741593452062"):
        dates = d.get("UF_CRM_1741593408621");
        out["december_2"] = {"dates": dates if isinstance(dates, list) else ([dates] if dates else []), "type": str(d.get("UF_CRM_1741593452062") or "")}
    return out

def _compute_periodicity(cleaning_dates: Dict[str, Any]) -> str:
    wash_dates = 0; sweep_dates = 0; full_wash_dates = 0; first_floor_wash_dates = 0
    for key in ["september_1","september_2"]:
        ...
        if has_wash: wash_dates += len(dates)
        if has_sweep: sweep_dates += len(dates)
        if has_wash and is_full: full_wash_dates += len(dates)
        if has_wash and is_first_floor: first_floor_wash_dates += len(dates)
...
async def get_houses(
    ...
    try:
        ...
        def ok(d):
            ...
            if cleaning_date:
                hit = any((dt == cleaning_date) for v in cd.values() if isinstance(v, dict) for dt in (v.get("dates", []) or []))
                if not hit:
                    return False
            if date_from and date_to:
                hit = any((date_from <= dt <= date_to) for v in cd.values() if isinstance(v, dict) for dt in (v.get("dates", []) or []))
                if not hit:
                    return False
            return True
...
# ========================= AI KNOWLEDGE (kept; does not affect CRM) =========================
...
async def ai_upload(files: List[UploadFile] = File(...), chunk_tokens: int = Form(1200), overlap: int = Form(200)):
    ...
    if AsyncSessionLocal is None and DATABASE_URL:
        raise HTTPException(status_code=500, detail='Database is not initialized')
    ...
    if not all_text.strip():
        ...
        if AsyncSessionLocal:
            async with AsyncSessionLocal() as s:
                meta = {'filenames':[f.filename for f in files],'summary':preview,'chunks':[],'chunks_count':0,'total_size_bytes':total_size,'total_pages':total_pages,'file_stats':file_stats,'chunk_tokens':int(chunk_tokens),'overlap':int(overlap)}
    ...
    preview = await _summarize(all_text) or (all_text[:500])
...
async def ai_save(upload_id: str = Form(...), filename: str = Form('document.txt'), db: AsyncSession = Depends(get_db)):
    ...
    for idx, (text, v) in enumerate(zip(chunks, vectors)):
        await db.execute(sa_text('INSERT INTO ai_chunks (id, document_id, chunk_index, content, embedding) VALUES (:i,:d,:x,:c,:e)'), {"i": str(uuid4()), "d": doc_id, "x": idx, "c": text, "e": v})
