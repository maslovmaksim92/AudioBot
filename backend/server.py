...
try:
    ...
    try:
        ...
        # Попытка импорта для локальной разработки (uvicorn server:app из backend/)
        from app.routers import health, auth, houses, cleaning, telegram, dashboard, logs, ai_knowledge, tasks, meetings, notifications, employees, ai_agent, ai_chat, finances, finance_transactions, finance_articles, revenue, debts, inventory, agents, telegram_webhook, agent_dashboard, render_logs, plannerka, realtime_transcription, telegram_auth, call_summary, bitrix_calls, bitrix_webhook, test_agent, debug_users, openai_realtime
        logger.info('✅ Routers imported via app.routers')
    except Exception as e2:
        logger.error(f'⚠️ Could not mount new routers: {e2}')
        import traceback
        logger.error(f'Traceback: {traceback.format_exc()}')
        health = auth = houses = cleaning = telegram = dashboard = logs = ai_knowledge = tasks = meetings = notifications = employees = ai_agent = ai_chat = finances = finance_transactions = finance_articles = revenue = debts = inventory = agents = telegram_webhook = agent_dashboard = render_logs = plannerka = realtime_transcription = None
