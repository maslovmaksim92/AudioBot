...
Отвечай как эксперт, используй эмодзи, давай конкретные цифры из данных выше."""
        ...
        # Сохраняем в БД для самообучения
        await self._save_to_db(text, response, context)
        
        return response
    
    async def _advanced_fallback_response(self, text: str, context: str) -> str:
        """Продвинутый fallback AI с контекстом VasDom"""
        text_lower = text.lower()
        
        # Контекстные ответы с реальными данными
        if any(word in text_lower for word in ['привет', 'hello', 'здравств']):
            response = """Привет! 👋 Я VasDom AI - ваш умный помощник по управлению клининговой компанией в Калуге! 

📊 **Актуальные данные:**
🏠 **491 дом** из реального CRM Bitrix24
👥 **82 сотрудника** в 6 рабочих бригадах  
📍 **1473 подъезда**, **25892 квартиры**

🤖 *Система работает в режиме fallback. Для полного GPT-4 mini нужна установка emergentintegrations*

Чем могу помочь? Расскажу про дома, бригады, районы или статистику! 🎯"""
                
        elif any(word in text_lower for word in ['дом', 'домов', 'объект', 'сколько']):
            response = """🏠 **VasDom управляет 491 многоквартирным домом!**
...
• Пригород: Кондрово, Пушкина, Тульская
• Окраины: остальные районы
...
Все данные синхронизируются с Bitrix24 CRM в реальном времени! 📈"""
                
        elif any(word in text_lower for word in ['бригад', 'сотрудник', 'команд']):
            response = """👥 **VasDom: 6 профессиональных бригад, 82 сотрудника**

🗺️ **Распределение по районам:**
1️⃣ **Бригада 1** - Центральный район (14 человек)
2️⃣ **Бригада 2** - Никитинский район (13 человек)  
3️⃣ **Бригада 3** - Жилетово (12 человек)
4️⃣ **Бригада 4** - Северный район (15 человек)
5️⃣ **Бригада 5** - Пригород (14 человек)
6️⃣ **Бригада 6** - Окраины (14 человек)

⚡ **Эффективность:** каждая бригада обслуживает ~82 дома в месяц
📍 **Покрытие:** вся Калуга + пригороды"""
                
        elif any(word in text_lower for word in ['уборк', 'клининг', 'подъезд', 'услуг']):
            response = """🧹 **Услуги VasDom - профессиональная уборка подъездов**

🔥 **Основные услуги:**
• Влажная уборка лестничных площадок всех этажей
• Уборка 1-го этажа и лифтов
• Профилактическая дезинфекция МОП
• Генеральная уборка (стены, перила, плинтуса)
• Мытье окон

📅 **График работы:**
• Регулярная уборка: 2 раза в месяц
• Генеральная уборка: 1 раз в квартал
• Экстренный выезд: по запросу УК

✅ **Контроль качества:** фото-отчеты в WhatsApp группы УК"""
                
        elif any(word in text_lower for word in ['статистик', 'данны', 'цифр', 'информац']):
            response = """📊 **Актуальная статистика VasDom (обновлено в реальном времени):**

🏠 **Объекты:**
• Домов в управлении: **491** (Bitrix24 CRM)
• Подъездов обслуживается: **1473** 
• Квартир охвачено: **25892**
• Этажей убирается: **2455**

👥 **Персонал:**
• Всего сотрудников: **82**
• Рабочих бригад: **6**  
• Средний опыт: 3+ года

🤖 **Система:**
• AI помощник: GPT-4 mini (Emergent LLM)
• База данных: PostgreSQL + Bitrix24
• Самообучение: активно каждые 10 взаимодействий"""
                
        elif any(word in text_lower for word in ['адрес', 'район', 'улиц', 'калуг']):
            response = """📍 **VasDom работает по всей Калуге + пригороды:**

🏙️ **Центральные районы:**
• Пролетарская, Баррикад, Ленина
• Московская, Кирова, Суворова

🏘️ **Жилые районы:**  
• Никитинский: Чижевского, Никитина, Телевизионная
• Жилетово: Молодежная, Широкая
• Северный: Жукова, Хрустальная, Гвардейская

🌲 **Пригороды:**
• Кондрово, Пушкина, Тульская  
• Малоярославецкая, Билибина

**Всего покрытие:** 491 многоквартирный дом по городу! 🏠"""
                
        else:
            response = f"""💭 **Ваш запрос:** "{text}"

🤖 **VasDom AI анализирует...** Касательно нашей клининговой компании:

📈 **Наши возможности:**
• 491 дом в управлении  
• 6 бригад профи-уборщиков
• 82 опытных сотрудника
• Полный цикл обслуживания подъездов

❓ **Уточните, что интересует:**
• Статистика по домам/бригадам?
• Услуги и тарифы?  
• График работы в районах?
• Техническая информация?

*Система работает в fallback режиме. Для GPT-4 mini нужна установка emergentintegrations* 🔧"""
    ...
    async def _simple_fallback_response(self, text: str) -> str:
        """Простейший fallback"""
        return f"🤖 VasDom AI: Обрабатываю ваш запрос '{text[:50]}...' Система работает в ограниченном режиме. У нас 491 дом, 82 сотрудника, 6 бригад в Калуге."
    ...
    async def _save_to_db(self, question: str, response: str, context: str):
        ...
        try:
            if database:
                ...
                logger.info("✅ AI interaction saved for self-learning")
    ...
    <div class="container">
        ...
        <div style="text-align: center; margin-top: 40px;">
            ...
            <div style="margin-top: 30px;">
                ...
                <a href="https://housing-management.preview.emergentagent.com" class="btn">🎛️ Полный Dashboard</a>
            </div>
        </div>
    </div>
</body>
