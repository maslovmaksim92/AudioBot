#!/usr/bin/env python3
"""
VasDom AudioBot Backend API Testing - Comprehensive Review Request Testing
...
class VasDomComprehensiveTester:
    ...
    def test_ai_db_dsn(self):
        ...
        if success and status == 200:
            ...
            query_ok = 'sslmode=require' in str(query)
            
            if scheme_ok and query_ok and raw_present:
                self.log_test("AI DB DSN - Requirements", True, 
                ...
                self.log_test("AI DB DSN - Requirements", False, f"❌ Issues: {', '.join(issues)}")
        else:
            self.log_test("AI DB DSN - Requirements", False, f"❌ Status: {status}, Data: {data}")
    ...
    def test_ai_db_check(self):
        ...
        if success and status == 200:
            ...
            if connected and tables_ok and dims_ok:
                self.log_test("AI DB Check - Requirements", True, 
                ...
                self.log_test("AI DB Check - Requirements", False, f"❌ Issues: {', '.join(issues)}")
                return False
        else:
            self.log_test("AI DB Check - Requirements", False, f"❌ Status: {status}, Data: {data}")
            return False
    ...
    def test_ai_preview(self):
        ...
        if success and status == 200:
            ...
            if upload_id and chunks > 0 and total_size_bytes > 0:
                self.log_test("AI Preview - Requirements", True, 
                ...
                self.log_test("AI Preview - Requirements", False, f"❌ Issues: {', '.join(issues)}")
        else:
            self.log_test("AI Preview - Requirements", False, f"❌ Status: {status}, Data: {data}")
    ...
    def test_ai_status(self, upload_id):
        ...
        if success and status == 200:
            ...
            if upload_status == 'ready':
                self.log_test("AI Status - Requirements", True, "✅ status='ready' ✓")
            else:
                self.log_test("AI Status - Requirements", False, f"❌ status='{upload_status}' (expected 'ready')")
        else:
            self.log_test("AI Status - Requirements", False, f"❌ Status: {status}, Data: {data}")
    ...
    def test_ai_study(self, upload_id):
        ...
        if success and status == 200:
            ...
            if document_id and chunks >= 1 and category == 'Маркетинг':
                self.log_test("AI Study - Requirements", True, 
                ...
                self.log_test("AI Study - Requirements", False, f"❌ Issues: {', '.join(issues)}")
        else:
            self.log_test("AI Study - Requirements", False, f"❌ Status: {status}, Data: {data}")
        ...
        if success and status == 200:
            ...
            if documents:
                ...
                if test_doc:
                    ...
                    if chunks_count >= 1:
                        self.log_test("AI Documents - Requirements", True, 
                                    f"✅ Found 'psycopg3.txt' with chunks_count: {chunks_count} ✓")
                    else:
                        self.log_test("AI Documents - Requirements", False, 
                                    f"❌ Document found but chunks_count={chunks_count} (expected >=1)")
                else:
                    self.log_test("AI Documents - Requirements", False, 
                                "❌ Test document 'psycopg3.txt' not found in documents list")
            else:
                self.log_test("AI Documents - Requirements", False, "❌ No documents found")
        else:
            self.log_test("AI Documents - Requirements", False, f"❌ Status: {status}, Data: {data}")
    ...
    def test_ai_search(self):
        ...
        if success and status == 200:
            ...
            if isinstance(results, list) and len(results) > 0:
                self.log_test("AI Search - Requirements", True, 
                ...
                if isinstance(results, list):
                    self.log_test("AI Search - Requirements", False, f"❌ results[] is empty (expected не пуст)")
                else:
                    self.log_test("AI Search - Requirements", False, f"❌ results should be array, got {type(results)}")
        elif status == 500:
            # Handle database connection issues gracefully
            self.log_test("AI Search - Requirements", False, f"❌ Status: {status} - Database connection issue (expected due to SSL errors)")
        else:
            self.log_test("AI Search - Requirements", False, f"❌ Status: {status}, Data: {data}")

    def test_ai_delete(self, document_id):
        ...
        if success and status == 200:
            ...
            if ok is True:
                self.log_test("AI Delete - Requirements", True, "✅ Document deleted successfully {ok:true} ✓")
            else:
                self.log_test("AI Delete - Requirements", False, f"❌ Expected ok=true, got ok={ok}")
        else:
            self.log_test("AI Delete - Requirements", False, f"❌ Status: {status}, Data: {data}")
    ...
    def run_comprehensive_tests(self):
        ...
        # Test 2: GET /api/ai-knowledge/db-check
        db_connected = self.test_ai_db_check()
        ...
        if upload_id:
            ...
            if document_id:
                ...
                # Test 8: DELETE /api/ai-knowledge/document/{document_id}
                self.test_ai_delete(document_id)
            else:
        ...
        # Test 9: GET /api/cleaning/filters
        self.test_crm_filters()
        
        # Test 10: GET /api/cleaning/houses?page=1&limit=5
        self.test_crm_houses()
