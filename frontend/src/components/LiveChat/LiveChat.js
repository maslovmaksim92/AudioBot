  ...
  const initWebSocket = () => {
    try {
      ...
      ws.onopen = () => {
        ...
        // Отправляем приветствие от Алисы
        setTimeout(() => {
          const welcomeMessage = {
            type: 'ai',
            text: 'Привет! Меня зовут Алиса, и я рада быть вашим персональным помощником в VasDom! Я знаю все о наших 348 домах и всегда готова помочь. Расскажите, что вас интересует?',
            timestamp: new Date()
          };
  ...
  const handleMessage = async (text) => {
    ...
    if (wsConnection && wsConnection.readyState === WebSocket.OPEN) {
      ...
      try {
        ...
        setMessages(prev => [...prev, {
          type: 'ai',
          text: 'Извините, что-то пошло не так 😔 Попробуйте еще раз, пожалуйста.',
          timestamp: new Date(),
          isError: true
        }]);
      }
